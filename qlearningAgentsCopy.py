...
class QLearningAgent(ReinforcementAgent):
	...
	def computeActionFromQValues(self, state):
		...
		if (max_value > 0):
			...
			# if all values are negative then unseen action will be optimal  
			unseen_action = list(set(self.getLegalActions(state)) - set(seen_action))
			if (len(unseen_action) > 0):
				return random.choice(unseen_action)
			else :
				return max_action
		util.raiseNotDefined()
	...
	def update(self, state, action, nextState, reward):
		...
		"""
		# first of all we find maxQ(nextstate ,action) over action
